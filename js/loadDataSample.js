...
const loadTasks = (tasks) => {
    ...
    for (const { title, description, subtasks } of tasks) {
        const id = generateId()
        ids.push(id)
        ...
        const data = {
            title,
            description,
            subtasksIds: loadSubtasks(subtasks)
        }
...
const loadBoards = () => {
    ...
    for (const { name, columns } of boards) {
        const id = generateId()
        ids.push(id)

        const data = {
            name,
            columnsIds: loadColumns(columns)
        }
